    ...
    def _update_duration_matrix(self, distance_matrix: torch.Tensor, current_time: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if self.rng is not None:
                normal_sample = torch.normal(mu.expand_as(distance_matrix), sigma.expand_as(distance_matrix), generator=self.rng)
                rand_factor = torch.exp(normal_sample)
            else:
            ...
            if self.rng is not None:
                num_accidents = torch.poisson(acc_rate.expand_as(distance_matrix), generator=self.rng)
            else:
